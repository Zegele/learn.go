...
func insertNode(root *Node, newNode *Node) *Node {
	...
	if newNode.data == root.data { //如果相等就保持原来的。不做变化。因为树的数是有序的。相等没法有序
		return root
	}

	if newNode.data < root.data {
		if root.left == nil { //找到正确的空位插入
			//找到位置，插入数据
			root.left = newNode
			newNode.root = root
		} else {
...
func findNextGenFromLeft(root *Node) *Node {
	...
	for {
		...
		} else { //如果tmpNode.right==nil的时候，就是要找的后继
			break
		}
	}
...
func findNextGenFromRight(root *Node) *Node {
	...
	return tmpNode //return的tmpNode没有left
}
