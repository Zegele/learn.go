...
import (
	"fmt"
	ren2 "learn.go/zuoye/dianti/diantikuozhan2/ren"
	"time"
)
...
func (d *DianTiXiang) JoinUpAndDown() {
	for _, valu := range d.NewReqFloorSlice[1:] { //[4,3,5]-> [3,4,5]  [5,1,2,4] --> [1,2,4,5]
		if valu > d.MostWantF {
			d.ToUpFloorSlice = append(d.ToUpFloorSlice, valu)
		} else if valu < d.MostWantF { //todo
	...
	fmt.Println("查看WantFloorSlice是否改变：", d.Ren.WantFloorSlice)
	for _, valu := range d.Ren.WantFloorSlice {
		if valu > d.MostWantF {
			d.ToUpFloorSlice = append(d.ToUpFloorSlice, valu)
		} else if valu < d.MostWantF {
			d.ToDownFloorSlice = append(d.ToDownFloorSlice, valu)
		}
	}
...
func (d *DianTiXiang) NewMoveTo() {
	if d.FangXiang == "ToUp" {
		fmt.Println("电梯向上运行>>>")
		//先运行到第一个请求电梯的地方。
		for chaJiLou := d.NewReqFloorSlice[0] - d.AtFloor; chaJiLou > 0; chaJiLou-- {
			d.AtFloor++
			d.SayJiFloor()
		}
		d.DianTiDoor()
		...
		//运行目标最多的楼层
		if d.MostWantF > d.AtFloor {
			for chaJiLou := d.MostWantF - d.AtFloor; chaJiLou > 0; chaJiLou-- {
				d.AtFloor++
				d.SayJiFloor()
			}
			fmt.Printf("第%d层，下电梯的人数最多！！！", d.MostWantF)
			d.DianTiDoor()
		} else {
			d.FangXiang = "ToDown"
			fmt.Println("电梯向下运行>>>")
		...
		fmt.Println("电梯向下运行>>>")
		for chaJiLou := d.AtFloor - d.NewReqFloorSlice[0]; chaJiLou > 0; chaJiLou-- {
			d.AtFloor--
			d.SayJiFloor()
		}
		d.DianTiDoor()
...
func (d *DianTiXiang) MostWant() { //获取目标楼层中人数最多的那层
	...
	fmt.Println("MostWantFloors:", d.MostWantFloors)
	d.SelectMostWant()
	fmt.Println("mostWant:", d.MostWantF)
	d.QuChuMostFloor()
	fmt.Println("去重后的ren.wantfloorslice:", d.Ren.WantFloorSlice)
}

func (d *DianTiXiang) SelectMostWant() { //todo 这个有问题？？？
	if d.FangXiang == "ToUp" {
		d.MostWantFloors = SliceSmallToBig(d.MostWantFloors) //从小到大排序
		for _, v := range d.MostWantFloors {
			if v > d.NewReqFloorSlice[0] {
				d.MostWantF = v
		...
		if d.MostWantF == 0 {
			d.MostWantFloors = SliceSmallToBig(d.MostWantFloors) //从大到小排序
			for _, v := range d.MostWantFloors {
				if v > d.NewReqFloorSlice[0] {
					d.MostWantF = v
					fmt.Println("d.MostWantF, v :", d.MostWantF, v)
...
//在wantFloorSlice中去除mostfloor
func (d *DianTiXiang) QuChuMostFloor() {
	fmt.Println("原始", d.Ren.WantFloorSlice)
	tempSlice := []int{}
	for j := 0; j < len(d.Ren.WantFloorSlice); j++ {
		if d.Ren.WantFloorSlice[j] != d.MostWantF {
			tempSlice = append(tempSlice, d.Ren.WantFloorSlice[j])
			fmt.Println("jhk", tempSlice)
		}
	}
	d.Ren.WantFloorSlice = tempSlice
	fmt.Println("去除后的", d.Ren.WantFloorSlice)
}
