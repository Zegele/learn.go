...
func main() {
	...
	defer cancel()
	successFlag := make(chan bool, 1) //用于传递成功的信号
	...
	case v := <-successFlag:
		if v {
			...
			fmt.Println("任务失败，需要重新考虑重试，或下线服务。")
		}
	}
}

func account(ctx context.Context) {
	ctx, cancel := context.WithCancel(ctx) //在ctx（timeout）上，衍生了新的ctx(cancel)
	defer cancel()
...
func configure(_ context.Context) { //这里忽略了ctx参数
	fmt.Println("注入新服务账号")
}

func verifyService(ctx context.Context, flag chan bool) {
	ctx, cancel := context.WithCancel(ctx) //衍生了
	defer cancel()
